...
(async () => {
  ...
  try {
    ...
    // 6. 点击开始游戏（房主可见）
    console.log('点击开始游戏...');
    const startButton = await page.locator('button').filter({ hasText: /开始游戏/ }).first();
    if (await startButton.isVisible().catch(() => false)) {
      if (await startButton.isEnabled().catch(() => false)) {
        await startButton.click();
        console.log('游戏开始！');
      } else {
        console.log('开始游戏按钮未启用（可能有玩家未准备）');
      }
    }
    
    await wait(4000);
    await page.screenshot({ path: 'game-ui-5-gameplay.png' });
    console.log('✓ 游戏界面截图已保存');
    
    // 7. 等待游戏进行
    await wait(15000);
    await page.screenshot({ path: 'game-ui-6-playing.png' });
    console.log('✓ 游戏进行中截图已保存');
    
    // 8. 再等待一段时间看吃碰杠胡按钮
    await wait(10000);
    await page.screenshot({ path: 'game-ui-7-actions.png' });
