...
export function broadcastGameState(io: Server, roomId: string, roomManager: RoomManager): void {
  ...
  for (const player of room.players) {
    ...
    if (player.type === 'human') {
      // 人类玩家：发送 game:state
      console.log(`[broadcastGameState] 人类玩家 ${player.name}(${player.id.slice(0,8)}) 需要发送状态`);
      
      // 调试：打印所有 socket 信息
      const allSocketsData = Array.from(io.sockets.sockets.values()).map(s => ({
        id: s.id.slice(0, 8),
        playerId: s.data.playerId?.slice(0, 8),
        roomId: s.data.roomId?.slice(0, 8),
      }));
      console.log(`[broadcastGameState] 当前所有 sockets:`, JSON.stringify(allSocketsData));
      
      // 方式1: 通过 playerId 查找
      let foundSocket: Socket | undefined = Array.from(io.sockets.sockets.values())
        .find(s => s.data.playerId === player.id);
      
      console.log(`[broadcastGameState] 方式1查找结果: ${foundSocket ? '找到' : '未找到'}`);
      
      // 方式2: 如果没找到，直接广播到房间（所有人都能收到）
      if (!foundSocket) {
        console.log(`[broadcastGameState] 方式2: 广播到房间 ${roomId}`);
...
function executeAIDecision(
  ...
  switch (decision.action) {
    ...
    case 'hu':
      ...
      console.log(`[AI] 执行 ${decision.action}, tiles:`, fullTiles?.map(t => t.display));
      ...
      const result = room.gameEngine.performAction(playerId, pendingAction);
      console.log(`[AI] performAction ${decision.action} 结果:`, result);
      
      // 如果操作失败，自动跳过
      if (!result) {
        console.log(`[AI] ${decision.action} 失败，自动跳过`);
  ...
  for (const [playerId, actions] of playerActionsMap) {
    ...
    if (player?.type === 'ai-agent' || player?.type === 'npc') {
      ...
      if (adapter) {
        ...
        // 没有 adapter，自动跳过
        console.log(`[handleAIActions] ${player.name} 无 adapter，自动跳过`);
        // 延迟 1 秒后跳过
        setTimeout(() => {
          const currentRoom = roomManager.getRoom(roomId);
...
export async function handleReady(
  ...
  try {
    ...
    if (serverRoom) {
      const clientRoom = toClientRoom(serverRoom);
      const allReady = clientRoom.players.length >= 4 && clientRoom.players.every(p => p.isReady);
      console.log(`[Server] 广播 room:updated, 玩家数=${clientRoom.players.length}, 全部准备=${allReady}`);
      // 广播给房间所有人
      io.in(roomId).emit('room:updated', { room: clientRoom });
      
      // 自动开始游戏：如果所有人准备好了，且房主是 AI（不是人类）
      if (allReady) {
          ...
          setTimeout(() => {
            ...
            if (startRoom && (startRoom.state === 'waiting' || startRoom.state === 'finished')) {
              ...
              if (gameRoom) {
                ...
                console.log(`[Server] AI 房主自动开始游戏成功`);
                io.in(roomId).emit('game:started', { roomId });
                broadcastGameState(io, roomId, roomManager);
              }
            }
          }, 1000);
...
export async function handleJoinAI(
  ...
  try {
    ...
    if (serverRoom) {
      ...
      // 检查是否应该自动开始游戏
      const allReady = serverRoom.players.length >= 4 && serverRoom.players.every(p => p.isReady);
      if (allReady) {
        const host = serverRoom.players.find(p => p.id === serverRoom.host);
        console.log(`[Server] AI加入后检查自动开始: 房主=${host?.name}, 类型=${host?.type}, 全员准备=${allReady}`);
        if (host && host.type !== 'human') {
          console.log(`[Server] AI 房主 ${host.name} 检测到全员准备，自动开始游戏`);
          setTimeout(() => {
            const startRoom = roomManager.getRoom(roomId);
...
export async function handlePlayerSpeak(
  ...
  try {
    ...
    const aiPlayers = room.players.filter(
      p => (p.type === 'ai-agent' || p.type === 'npc') && p.id !== playerId
    );
    
    if (aiPlayers.length > 0) {
      // 获取 LLM 配置
      let llmConfig: { apiKey?: string; endpoint?: string; model?: string; providerType?: string } = {};
      try {
        const fs = require('fs');
        ...
        if (fs.existsSync(configPath)) {
          ...
          llmConfig = {
            ...
            endpoint: configData.llm?.apiBase ? `${configData.llm.apiBase}/chat/completions` : undefined,
            model: configData.llm?.model,
            providerType: configData.llm?.type || configData.llm?.providerType,
          };
        }
      } catch (e) {
        console.log('[Conversation] 读取 LLM 配置失败');
      }
      
      // 触发会话层即时回应（不阻塞），传入所有玩家信息
      const conversationManager = getConversationManager(io, roomId);
      conversationManager.handleSpeech(roomId, message, aiPlayers, llmConfig);
    }
...
/**
 * 处理玩家发言
 * 当人类玩家或 AI 玩家发送聊天消息时，广播给所有人并触发 AI 回应
 */
export async function handlePlayerSpeech(
  io: Server,
  socket: Socket,
  roomManager: RoomManager,
  ...
  try {
    ...
    if (!roomId || !playerId) {
      if (callback) callback({ success: false, error: '未加入房间' });
      return;
    }
    ...
    if (!sender) {
      if (callback) callback({ success: false, error: '玩家不存在' });
      return;
    }

    const content = payload.content?.trim();
    if (!content) {
      if (callback) callback({ success: false, error: '发言内容不能为空' });
      return;
    }
    ...
    // 广播给房间内所有玩家（包括发送者自己）
    io.in(roomId).emit('player:speech', message);
    ...
    if (allAIPlayers.length > 0) {
      ...
      const llmConfig = {
        ...
        model: firstAI.aiConfig?.llmModel,
        providerType: firstAI.aiConfig?.llmProviderType,
      };
    ...
    // 将发言也推送到所有 AI 的事件队列，让 AIAdapter 也能收到
    const { eventQueueManager } = require('../ai/EventQueue');
    for (const p of room.players) {
      if ((p.type === 'ai-agent' || p.type === 'npc') && p.id !== playerId) {
        eventQueueManager.pushTo(p.id, {
          type: 'player_speak',
          timestamp: Date.now(),
          data: {
            playerId,
