...
(async () => {
  ...
  try {
    ...
    // 3. 输入玩家名称并创建（使用更精确的选择器）
    await page.fill('input[placeholder="请输入名称..."]', '测试玩家');
    await page.waitForTimeout(500);
    
    // 点击模态框中的"创建"按钮（粉色/红色的那个）
    // 使用 nth(1) 来选择第二个"创建"按钮（模态框中的）
    await page.locator('button:has-text("创建").primary-btn, button:has-text("创建") >> nth=1').click();
    await page.waitForTimeout(2000);
    await page.screenshot({ path: 'game-ui-3-room.png' });
    console.log('✓ 房间截图已保存');
    
    // 4. 点击"准备"按钮开始游戏
    await page.click('button.ready-btn');
    await page.waitForTimeout(2000);
    
    // 由于是单人测试，需要等待 AI 加入并准备
    // 等待更长时间让 AI 自动加入
